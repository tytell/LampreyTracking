...
datadir <- file.path(boxdir,'2021/Processed Data/Exp_080321')
```
...
```{r}
datafilenames <- datafilenames[str_detect(datafilenames, '080321')]
```
...
data <- purrr::map_dfr(datafilenames, ~ load_katz_data(.x, scales,
                                                       widthdata = width, fps = fps,
                 bodypartorder = bodypartorder, showfile = TRUE))
write_csv(data, file.path(boxdir,'2021/Processed Data/A20803data.csv'))
```

This loads the previous processed data file, as produced above.
```{r}
data <- read_csv(file.path(boxdir,'2021/Processed Data/A20803data.csv'),
...
data.nooutliers %>%
  ...
  #filter(trial == '080321_A2V6_04' | trial == '080321_A2V6_13') %>%
  #filter(trial == '080321_A2V6_13') %>%
  group_by(trial, frame) %>%
  #filter(bodyparts == 'tailbase') %>%
  ggplot(aes(x = xmm, y = ymm)) +
  geom_line(aes(group = frame), color = 'black') +
...
data.nooutliers <-
  data.nooutliers %>%
  get_point_order()
write_csv(data.nooutliers, file.path(boxdir,'2021/Processed Data/A20803data.csv'))
```
...
data <-
  ...
  group_by(filename) %>%
  group_modify(~ get_all_kinematics(.x, widthdata = width))
write_csv(data, file.path(boxdir,'2021/Processed Data/A20803data-goodframes.csv'))
```
...
```{r}
data <- read_csv(file.path(boxdir,'2021/Processed Data/A20803data-goodframes.csv'),
                 col_types = list(Animal = col_character(),
                                  Treatment = col_factor(levels = treatments),
                                  bodyparts = col_factor(levels = bodypartorder),
                                  curve = col_number()))
