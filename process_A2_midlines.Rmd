...
```{r}
data %>%
  filter(trial == '062921_A2V10_03') %>%
  filter(bodyparts == 'tailbase') %>%
  #group_by(frame) %>%
  plot_ly(x = ~t, y = ~curves, type = 'scatter', mode = 'lines+markers') %>%
  add_markers(x = ~t, y = ~curves, color = ~ cycle_step)
```

```{r}
q <-data %>%
  select(-(dx:wavelength)) %>%
  filter(trial == '062921_A2V10_05') %>%
  get_arc_length() %>%
...
q %>%
  ...
  #filter(frame == 52) %>%
  group_by(frame) %>%
  get_body_axis_in_frame() %>%
  ggplot(aes(x = xmm, y = ymm, color = frame)) +
  geom_path(aes(group = frame)) +
  geom_segment(
               aes(x = comx, y = comy, xend = comx + len/2 * bodyaxisx, yend = comy + len/2 * bodyaxisy),
               inherit.aes = FALSE) +
  coord_fixed()
```
...
q %>%
  ...
  #filter(bodyparts == 'tailbase') %>%
  #pull(curve)
  plot_ly(x = ~frame, y = ~curve, color = ~bodyparts, type = 'scatter', mode = 'lines+markers')
```

```{r}
p1 <- q %>% filter(bodyparts == 'tailbase') %>%
  #group_by(frame) %>%
  plot_ly(x = ~t, y = ~tracks, type = 'scatter', mode = 'lines+markers') %>%
  add_markers(x = ~t, y = ~tracks, color = ~ cycle_step)

p2 <- q %>% filter(bodyparts == 'tailbase') %>%
  filter(peak != 0) %>%
  plot_ly(x = ~t, y = ~ peaksize, type = 'scatter', mode = 'lines+markers')

p3 <- q %>% filter(bodyparts == 'tailbase') %>%
  filter(peak != 0) %>%
  plot_ly(x = ~t, y = ~ peakgap, type = 'scatter', mode = 'lines+markers')
